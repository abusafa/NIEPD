...
export default function CreateNavigationPage() {
  ...
  useEffect(() => {
    if (navigationState.items && formData.location) {
      const itemsInLocation = navigationState.items.filter(
        item => item.location === formData.location && !item.parentId
      );
      const maxOrder = Math.max(0, ...itemsInLocation.map(item => item.sortOrder));
      setFormData(prev => ({ ...prev, sortOrder: maxOrder + 1 }));
    }
  }, [formData.location, navigationState.items]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Failed to create navigation item');
      }
    } catch (error) {
  ...
  // Filter parent options based on location
  ...
  return (
    <FormLayout
      ...
      description="Add a new menu item to your site navigation"
      onBack={handleBack}
      onSave={handleSave}
      loading={loading}
    >
