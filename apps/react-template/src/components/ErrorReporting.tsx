...
export const ErrorReportModal: React.FC<ErrorReportModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For react-template, we'll send directly to the website API
      const apiUrl = process.env.NEXT_PUBLIC_WEBSITE_API_URL || 'http://localhost:3000';
      const response = await fetch(`${apiUrl}/api/error-reports`, {
        method: 'POST',
        headers: {
